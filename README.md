...
> ⚠️ **免责声明（务必先读）**
> 本项目是**非官方**的技术学习项目，**仅供个人学习、技术研究与交流**，
> **严禁**用于私服运营、商业牟利、二次打包分发或传播游戏资源。
> 仓库内不含安装包、美术音频资源与提取的数据表；游戏全部权利归原权利人所有。
> 完整条款见 **[DISCLAIMER.md](DISCLAIMER.md)**；不同意者请立即停止使用并删除本项目。
...
> **本仓库的代码部分只包含服务端源码（协议实现 + 工具 + 笔记）**，不含美术/音频资源，也不含从客户端提取的数据表。
> 供学习用的**重打包离线客户端**放在 **[Releases](https://github.com/lingcat521/lxqw-offline-server/releases)**，
> 它是从作者合法持有的客户端改出来的测试版（已知问题见 Release 说明），请自行判断是否使用。
>
...
- 只做单机：排行榜、真实好友/分享、活动公告、账号类等需要真实服务端的玩法不实现（见 `联网功能清单.md`）。
- **内购/充值走本地模拟**（`new/iap.js`）：点一下直接发货，不联网、不产生真实交易，仅用于学习内购流程；
  请勿据此用于任何商业用途或规避官方付费内容。
- **新手教程自动跳过**（原教程流程有 bug，见 Release 说明）；日历界面存在显示异常；其余若干 bug 待处理。
- 存档是本机 JSON，不是真实服务端数据库，删掉即重开。
- 版本跟随 v1.0.20 客户端协议；客户端更新后协议可能需要重新对齐。

## 许可证

详见 **[DISCLAIMER.md](DISCLAIMER.md)**（使用范围与禁止用途）。
