...
function matchBrace(s,start){ let depth=0,k=start; while(k<s.length){ const c=s[k]; if(c==='{')depth++; else if(c==='}'){depth--; if(depth===0)return k;} else if(c==='"'||c==="'"){const q=c;k++; while(k<s.length&&s[k]!==q)k++;} k++; } return -1; }
const re=/prototype\.([A-Za-z0-9_$]+)=function\(([^)]*)\)\{/g;
const handlers=Object.create(null); let m;
while((m=re.exec(src))){ const name=m[1],params=m[2],bs=m.index+m[0].length-1,be=matchBrace(src,bs); if(be<0)continue; (handlers[name]=handlers[name]||[]).push({params,body:src.slice(bs+1,be)}); }
...
function makeRecorder(R, path){
  ...
  return new Proxy(t,{
    get(tt,k){
      ...
      if(k==='length'){ R.fields.add(path+'.length'); R.arrays.add(path); return 1; }
...
const markArr=function(x){ try{ try{ if(x&&(typeof x==='object'||typeof x==='function')&&x[PATH]!==undefined&&CUR) CUR.arrays.add(x[PATH]); }catch(e){} }catch(e){} };
let CUR=null;
function makeUtils(){ const base=makeAuto(); return new Proxy({}, { get(t,k){ if(k==='convertArray'||k==='convertArrayAll') return function(x){ markArr(x); return x; }; return base; }, has(){return true;} }); }
const out=Object.create(null);
for(const name of protos){
  const hs=handlers[name]; if(!hs)continue;
  const R={fields:new Set(),arrays:new Set()}; CUR=R; const req=new Set(); let err=null;
  for(const h of hs){
    const declared=declaredNames(h.body,h.params);
    const scope=new Proxy({},{ has:(t,k)=>typeof k==='string'&&!declared.has(k), get:(t,k)=>{ if(typeof k==='symbol')return undefined; if(k==='Array')return {isArray:(x)=>{ markArr(x); return true; },from:()=>auto}; if(k==='Object')return Object; if(k==='JSON')return JSON; if(k==='Math')return Math; if(k==='Utils')return makeUtils(); return auto; } });
    let fn=null;
    try{ fn=new Function('__scope','with(__scope){ return function('+h.params+'){'+h.body+'} }')(scope); }catch(e){ err='compile:'+e.message; continue; }
    const resp=makeRecorder(R,''); const rq=makeRecorder({fields:req,arrays:new Set()},'');
    try{ fn.call(auto, resp, rq); }catch(e){ if(!err)err='run:'+e.message; }
  }
  out[name]={fields:[...R.fields].sort(), arrays:[...R.arrays].sort(), req:req.size>0, err};
}
...
const keys=Object.keys(out).filter(k=>out[k].fields.length&&!out[k].err);
console.log('probed:',Object.keys(out).length,'clean:',keys.length);
console.log('errored:', Object.keys(out).filter(k=>out[k].err).length);
console.log('req-param protocols:', Object.keys(out).filter(k=>out[k].req).join(' '));
console.log();
for(const k of ['client_load_role','item_load_items','furniture_load_furniture','album_load','visit_load','weather_load','guest_load_drawing']) if(out[k]) console.log('### '+k+'\n  fields: '+out[k].fields.join(', ')+'\n  arrays: '+out[k].arrays.join(', '));
